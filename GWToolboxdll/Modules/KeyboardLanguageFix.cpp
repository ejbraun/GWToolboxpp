...
* Restores original keyboard layout after probing an already installed layout.
*
* THE ISSUE:
...
*
* THE SOLUTION:
* We don't actually want GW to install a language if its not found - reuse it if you have it, otherwise keep using the one that the OS is using instead.
*
* SIDE EFFECTS:
...
void KeyboardLanguageFix::Initialize()
{
    ...
    const auto operand = GW::Scanner::Find("\xa1????\x81\xe6\xff\xff\xff\x7f\x85\xc0", "x????xxxxxxxx", 1);
    const auto address = operand ? *reinterpret_cast<HKL**>(operand) : nullptr;
    const auto valid_address = address && GW::Scanner::IsValidPtr(reinterpret_cast<uintptr_t>(address))
        && GW::Scanner::IsValidPtr(reinterpret_cast<uintptr_t>(address) + sizeof(*address) - 1);
#ifdef _DEBUG
    ASSERT(valid_address && "Failed to find keyboard layout address");
#endif
    if (!valid_address) {
    ...
    if (!current_keyboard_layout) {
        Log::Error("Failed to get the current keyboard layout");
        return;
    }
    ...
    for (auto i = 0; i < keyboard_languages_count; i++) {
        ...
        const auto have_layout_name = GetKeyboardLayoutNameA(layout_name);
        const auto name_error = have_layout_name ? ERROR_SUCCESS : GetLastError();
        // Layout probing runs on the game thread; restore its input layout before either return path.
        if (!ActivateKeyboardLayout(current_keyboard_layout, 0)) {
            Log::Error("Failed to restore the keyboard layout, GetLastError = %#08x", GetLastError());
            return;
        }
        if (!have_layout_name) {
            Log::Error("Failed to GetKeyboardLayoutNameA, GetLastError = %#08x", name_error);
            return;
        }
