...
Hotkey definitions are shared by every config profile and saved immediately when edited. Character, profession, map, and instance filters still let one global hotkey list behave differently for each client or character. Use `/tb global save` or `/tb global load` for an explicit save or reload.
